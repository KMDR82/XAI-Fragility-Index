...
* **Dataset:** Download the RSNA Pneumonia Detection Challenge dataset from the official [Kaggle Repository](https://www.kaggle.com/competitions/rsna-pneumonia-detection-challenge). Ensure that the `stage_2_train_labels.csv` file and the image directory (containing `.dcm`, `.png`, or `.jpg` files) are located in your working directory or specified input path.
* **Model Weights:** Download the pre-trained ResNet-50 model weights (`best_pneumonia_model.pth`) from [this Google Drive link](https://drive.google.com/file/d/1Ae31_XiP_SsvWOnnItbOiYKPmTH31b_o/view?usp=sharing) and place the downloaded `.pth` file in the root directory of the project.
